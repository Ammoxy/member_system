<template>
  <div v-loading="loading" element-loading-text="拼命加载中">
    ...
    <el-dialog :visible.sync="dialogDocument" title="文档编辑" width="60%">
...
<script>
  ...
  export default {
    ...
    mounted() {
      this.getDocument(this.current, this.size);
    },
    methods: {
      // 获取资讯类型
      getDocument(cur, list) {
        var self = this;
        API.documents(cur, list)
          .then((res) => {
            self.loading = false;
      ...
      currentChange(val) {
        ...
        self.current = val;
        self.loading = true;
        self.getDocument(val, self.size);
      },
      // 每页多少条
      sizeChange(val) {
        var self = this;
      ...
      newDocument() {
        ...
        API.document(self.form).then((res) => {
          ...
          self.getDocument(self.current, self.size);
          self.form = {};
        });
      },
