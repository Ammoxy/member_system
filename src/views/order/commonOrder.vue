<template>
    <div v-loading="loading" element-loading-text="拼命加载中">
        ...
        <el-table :data="tableData" border :header-cell-style="{ background: '#f0f0f0' }" max-height="620">
            ...
            <el-table-column prop="user.nickname" label="微信名"></el-table-column>
            <el-table-column prop="userGood.img" label="头像">
            ...
            <el-table-column prop="total_money" label="商品总金额"></el-table-column>
                ...
                <template slot-scope="scope">
                    <el-dropdown>
                        ...
                        <el-dropdown-menu slot="dropdown">
                            ...
                            <el-dropdown-item>
                                <el-button size="mini" type="primary" @click="handleGood(scope.$index, scope.row)">
                                    订单商品
                                </el-button>
                            </el-dropdown-item>
        ...
        <el-dialog title="订单商品" :visible.sync="dialogGood" width="80%" center>
            <el-table :data="goodData" empty-text="暂无数据" border :header-cell-style="{ background: '#f0f0f0' }"
                max-height="620">
                <el-table-column prop="id" label="ID"></el-table-column>
                <el-table-column prop="good.name" label="商品名称"></el-table-column>
                <el-table-column prop="good.intro" label="简介"></el-table-column>
                <el-table-column prop="good.img" label="图片" width="150">
                    <template slot-scope="scope">
                        <div v-if="scope.row.good.img">
                            <el-popover placement="top-start" title trigger="click">
                                <img :src="scope.row.good.img" style="max-width: 800px; max-height: 800px" />
                                <img slot="reference" :src="scope.row.good.img"
                                    style="max-width: 180px; max-height: 80px" />
                            </el-popover>
                        </div>
                        <div v-else>
        ...
        <!-- 发货 -->
        <el-dialog :visible.sync="dialogShipments" title="是否发货" width="50%" align="center"
    ...
    export default {
        ...
        methods: {
            ...
            getOrderList(cur, list, status, no, good_id) {
                var self = this;
                self.loading = true;
                API.orders(cur, list, status, no, good_id)
                    .then((res) => {
                        self.loading = false;
            ...
            handleDetail(index, row) {
                ...
                if (self.permissionData.includes("distribution")) {
                    self.dialogOrder = true;
                    if (row.is_fetch == 1) {
                        API.shopingpInfo(self.order_id, 1).then(res => {
                            console.log(self.orderData);
                            self.orderData = res.result;
                            self.$message.success("获取数据成功");
                        })
                    } else if (row.is_fetch == 2) {
                        API.recaptionInfo(self.order_id, 1).then(res => {
                            self.orderData = res.result;
            ...
            handleGood(index, row) {
                var self = this;
                self.order_id = row.id;
                if (self.permissionData.includes("orderGood")) {
                    self.dialogGood = true;
                    API.ordersGoods(self.order_id).then(res => {
                        self.goodData = res.result;
                        self.$message.success("获取数据成功");
                    })
                } else {
                    self.$message.warning("无权操作");
            ...
            toConfirm() {
                ...
                if (orderNum && logistics_id) {
                    API.confirmShipments(self.order_id, 1, orderNum, logistics_id).then(res => {
            ...
            handleLogistics(index, row) {
                ...
                if (self.permissionData.includes("logisticsGood")) {
                    if (logistics_id > 0) {
